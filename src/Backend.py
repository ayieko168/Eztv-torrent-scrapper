...
from PyQt5.QtWidgets import QMainWindow, QHeaderView, QMessageBox, QTableWidgetItem, QApplication, QMenu, QLabel, QDialog, QHBoxLayout, QVBoxLayout
...
if (sys.platform == "win32") or (sys.platform == "cygwin"):
    resultPath = os.path.join(BASE_DIR, "utils\\resources\\result.json")
    EZTV_RFERENCE_DICTIONARYPath = os.path.join(BASE_DIR, "utils\\resources\\EZTV_RFERENCE_DICTIONARY.json")
    ref_Path = os.path.join(BASE_DIR, "utils\\resources\\ref_.json")
elif sys.platform == "linux":
    # print("plartform is linux")
    resultPath = os.path.join(BASE_DIR, "./utils/resources/result.json")
    EZTV_RFERENCE_DICTIONARYPath = os.path.join(BASE_DIR, "./utils/resources/EZTV_RFERENCE_DICTIONARY.json")
    # ref_Path = "./utils/resources/ref_.json"
    ref_Path = os.path.join(BASE_DIR, "utils/resources/EZTV_RFERENCE_DICTIONARY.json")
...
class App(QMainWindow):
    ...
    def change_scrapers_in_combo(self):
        ...
        if self.ui.searchForCombo.currentText().lower() == "TV-Show".lower():
            ...
            subs_scrapers = ['Sub Scene', 'Open Subs', 'YIFI Subs', 'Tv Subs .com', 'Tv Subs .net', 'All Sites']
            self.ui.scraperCombo.clear()
    ...
    def getDataCMD(self, signals):
        ...
        with open(os.path.join(BASE_DIR, 'utils/resources/result.json'), 'r') as resutlsFO:
            results_dictionary = json.load(resutlsFO)
        with open(os.path.join(BASE_DIR, 'utils/resources/EZTV_RFERENCE_DICTIONARY.json'), 'r') as eztv_ref_FO:
        ...
        if requested_scraper == 'eztv':
            ...
            torrents_dictionary, torrents_count, return_code = torrent_scapers.yify_movie_scraper(movie_title=title, signals=signals)

        elif (self.ui.searchForCombo.currentText().lower() == "Anime".lower()) and (self.ui.scraperCombo.currentText().lower() == "All Sites".lower()):

            ## Get results from all the anime sites
            signals.log_data.emit("application[anime]", "Using all anime sites (Nyaa, Anime Tosho, Kickass) to scrape for data...")

            signals.log_data.emit("application[anime]", "Starting Nyaa scraper...")
            torrents_dictionary1, torrents_count1, return_code1 = torrent_scrapers.nyaa_scraper(movie_title=title, signals=signals)
            signals.log_data.emit("application[anime]", f"Done with the Nyaa scraper, found {torrents_count1} torrents...")

            signals.log_data.emit("application[anime]", "Starting Anime Tosho scraper...")
            torrents_dictionary2, torrents_count2, return_code2 = torrent_scrapers.anime_tosho_scraper(movie_title=title, signals=signals)
            signals.log_data.emit("application[anime]", f"Done with the Anime Tosho scraper, found {torrents_count2} torrents...")

            signals.log_data.emit("application[anime]", "Starting Kickass scraper...")
            torrents_dictionary3, torrents_count3, return_code3 = torrent_scrapers.kick_ass_scraper_anime(movie_title=title, signals=signals)
            signals.log_data.emit("application[anime]", f"Done with the Kickass scraper, found {torrents_count3} torrents...")

            totla_torrents = torrents_count1 + torrents_count2 + torrents_count3
            signals.log_data.emit("application[anime]", f"Done with all scrapers, found {totla_torrents} total torrents")

            combined_torrent_dictionary = {**torrents_dictionary1, **torrents_dictionary2, **torrents_dictionary3}

            if return_code1 or return_code2 or return_code3:
                return_code = True
            else:
                return_code = False

            torrents_dictionary = combined_torrent_dictionary
            torrents_count = totla_torrents
        ...
        if return_code == True:
            ...
            with open(os.path.join(BASE_DIR, 'utils/resources/result.json'), 'w') as result_fo:
...
class PreWindow(QDialog):
    ...
    def __init__(self):
        ...
        self.infoLabel.setText("Please Wait while we setup things...")

        self.loadingGif = QLabel()
        loading_gif = QMovie("utils/imgs/loading.gif")
        self.loadingGif.setMovie(loading_gif)
        loading_gif.start()

        self.labelsLayout.addWidget(self.infoLabel, 0)
        self.labelsLayout.addWidget(self.loadingGif, 1)
